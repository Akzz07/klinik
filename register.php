...
if (isset($_POST['signUp'])) {
    ...
    $checkIdQuery = "SELECT * FROM dokter WHERE id='$id'";
    $result = $conn->query($checkIdQuery);
    if ($result->num_rows > 0) {
        echo "Dokter sudah terdaftar!";
    } else {
        // Masukkan data dokter baru ke database
        $insertQuery = "INSERT INTO dokter(id, nama, alamat, no_hp, id_poli)
    ...
    if ($id === $adminUsername && $alamat === $adminPassword) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($alamat === $row['alamat']) {
                $_SESSION['id'] = $row['id']; // Simpan ID user di sesi
                $_SESSION['role'] = 'user'; // Tandai sebagai user biasa
                header("Location: dokterpage.php"); // Arahkan ke homepage user
                exit();
            } else {
                echo "Password salah!";
            }
